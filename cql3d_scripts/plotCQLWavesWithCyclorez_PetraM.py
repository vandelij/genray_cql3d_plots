...
folder = 'scan_bmpwr_gen_D_gen_e_longer_rays'
cql_nc = netCDF4.Dataset(f'../shots/{shotNum}/{folder}/cql3d_bmpwr_{save_number_for_scan}.nc','r')
cqlrf_nc = netCDF4.Dataset(f'../shots/{shotNum}/{folder}/cql3d_krf_bmpwr_{save_number_for_scan}.nc','r')
save_folder_and_name = f'{folder}/rays_bmpwr{save_number_for_scan}.png'
...
ssibdry = gfileDict["ssibdry"]
psirz = gfileDict["psirz"]
ssimag = gfileDict["ssimag"]
npsirz = (psirz - ssimag) / (ssibdry - ssimag)
get_npsi_rz = interp2d(rgrid, zgrid, npsirz) # a function that can return the flux surface

print('Loading density from /home/vandelij/Desktop/HFS_HHFW_antenna/PetraM/genray_density.npy')
genray_density = np.load('/home/vandelij/Desktop/HFS_HHFW_antenna/PetraM/genray_density.npy')
genray_rho = np.linspace(0, 1, genray_density.shape[0])
get_denisty_from_flux = interp1d(genray_rho, genray_density, fill_value='extrapolate')
def get_density(r,z):
    npsi = get_npsi_rz(r,z)
    rho = np.sqrt(npsi)
    return get_denisty_from_flux(rho)
...
def plotRays(frequency, harmonics, species, r_resolution, z_resolution, levels):
    ...
    plotRcuttoff(ax, frequency, species, r_resolution, z_resolution)
    ax.legend()
...
def plotRcuttoff(ax, frequency, species, r_resolution, z_resolution):
    '''
    ax: plot to append to 
    frequency: launched wave fequency [Hz]
    harmonics: list of harmonics to plot (example: [1, 2, 3] will plot the 1st, second, and third cyclotron harmonics for species) 
    ...
    normalized_w_wave = w_wave / omega_j
    R, Z = np.meshgrid(r_points, z_points)
    density = get_density(r_points,z_points)
    nR_CO = omega_j**2*(normalized_w_wave + 1)*(6**2-1)
    n_ratio_nRCO = density.T / nR_CO

    CS = ax.contour(R, Z, n_ratio_nRCO.T, levels=[1], colors=('green',), linestyles=('-',))
    ax.clabel(CS, fmt = '%2.1d', colors = 'green', fontsize=12)
...
import pickle 
def drawPetraM(ax):
    with open('../../PetraM/data_dict.pkl', 'rb') as file:
        bdr_data = pickle.load(file)
    print(bdr_data.keys())
    xarray = bdr_data['vertices'][:, 0]
    yarray = bdr_data['vertices'][:, 1]
    data = np.real(bdr_data['data'])
    ax.tripcolor(xarray, yarray, data, vmin=-.08, vmax=0.08, shading='gouraud', cmap='bwr')
