...
gen_nc = netCDF4.Dataset(f'../shots/{shotNum}/longer_rays_test/genray.nc','r')
...
def plotRays():
    ...
    wr  = gen_nc.variables["wr"][:] #major radius of the ray at each point along the trace
    wz  = gen_nc.variables["wz"][:] #height of the ray at each point along the trace
    delpwr= gen_nc.variables["delpwr"][:] #power in the ray at each point
...
ax.xaxis.set_major_locator(FixedLocator([1, 1.2, 1.4, 1.6, 1.8, 2, 2.2]))

# Locations for the Y-axis, major grid
ax.yaxis.set_major_locator(FixedLocator([-1, -.8, -.6, -.4, -.2, 0, .2, .4, .6, .8, 1]))
#ax.grid()
plt.show()